...
for _ in range(q):
    ...
    if cmd[0] == 100:
        ...
        is_made[idx] = True
        dijkstra(start)
        profit = revenue-distance[to_]
        # if distance[to_]:profit = -int(1e9)
        heapq.heappush(s_heap, Stock(idx, revenue, to_, profit))
    elif cmd[0] == 400:
        ...
        nxt_s_heap = []
        while s_heap:
            nxt_s_heap.append(heapq.heappop(s_heap))
        for ele in nxt_s_heap:
            profit = ele.revenue - distance[ele.to_]
            # if distance[ele.to_] == int(1e9): profit = -int(1e9)
            # print(profit, "---")
            heapq.heappush(s_heap, Stock(ele.id, ele.revenue, ele.to_, profit))